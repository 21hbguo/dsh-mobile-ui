...
- **返回键退栈**（全屏 / PWA standalone 内）：系统返回键不再直接退出全屏或
  离开页面，而是按「最上层优先」逐层返回 —— aionui 对话框 → 模态对话框
  （设置页等）→ 侧栏抽屉 → 右侧面板；层全部关完后再按返回才退出全屏。
  Android 系统返回会先退全屏再弹历史，插件会关掉当前层并自动恢复全屏
  （粗指针设备生效；桌面 Esc 只关层不重进）。
