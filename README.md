...
- **返回键退栈 + 全屏锁定**（全屏 / PWA standalone 内）：系统返回键永不
  退出全屏、永不离开页面，而是按「最上层优先」逐层返回 —— aionui 对话框
  → 模态对话框（设置页等）→ 侧栏抽屉 → 右侧面板；层全关完后再按返回也
  只是吞掉（提示用 ⛶ 退出）。Android 系统返回会先退全屏再弹历史，插件
  会关掉当前层并立即恢复全屏（粗指针设备生效；连按返回同样每次都被吞掉；
  桌面 Esc 只关层不重进）。退出全屏唯一途径：右上角 ⛶ 按钮。
...
- 返回退栈：全屏/PWA 下 `history.pushState` 布陷阱，`popstate` = 系统返回键；
  层关闭后重新布陷阱（消费），空栈则退出全屏。Android 返回键会先触发
  `fullscreenchange` 退出，故另走「关层 + 防抖重进全屏」路径（粗指针设备）。
