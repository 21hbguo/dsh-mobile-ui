...
export class MobileUiEngine {
    ...
    // ---------- install ----------
    injectStyle() {
        if (document.getElementById(MOBILE_CSS_ID) !== null)
            return;
    ...
    applyBreakpoint() {
        ...
        const listener = () => { this.setActive(mq.matches); };
        this.mq = mq;
    ...
    attachFrame(frame) {
        ...
        this.styleObserver = new MutationObserver(() => {
            ...
            if (this.active && !this.autoClosed)
                this.autoCloseAionui();
        });
        ...
        frame.children[0]?.addEventListener('click', (event) => {
            ...
            if (button.hasAttribute('data-dsh-mobile-bar'))
                return;
    ...
    syncPanelButton() {
        if (this.barEl === null || this.frame === null)
            return;
    ...
    autoCloseAionui() {
        const frame = this.frame;
        if (frame === null || this.autoClosed || !this.active)
            return;
        try {
            ...
            if (!fiveTracks) {
                ...
                if (this.autoCloseGraceUntil === 0)
                    this.autoCloseGraceUntil = performance.now() + 2500;
            ...
            if (openCols.length === 0) {
                ...
                if (this.zeroStreakAt === 0)
                    this.zeroStreakAt = performance.now();
            ...
            if (this.autoCloseRetryAt === 0)
                this.autoCloseRetryAt = performance.now() + 8000;
            ...
            this.autoClosed = true; /* aionui 状态异常时保持现状 */
        }
    }
    // ---------- interactions ----------
    onBarAction(action) {
    ...
    onSessionsChange() {
        try {
            ...
            if (!changed || !this.active || this.frame === null)
                return;
